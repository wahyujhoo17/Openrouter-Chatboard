...
export async function POST(req: NextRequest) {
  try {
    ...
    const { message, attachmentUrl, history, model, conversationId } =
      await req.json();

    if ((!message || typeof message !== "string") && !attachmentUrl) {
    ...
    if (!canUseModel(requestedModel, userPlan)) {
      return NextResponse.json(
        {
          error:
            "Pro models require a Pro subscription. Please login and upgrade.",
    ...
    if (session) {
      ...
      if (conversationId) {
        ...
        convId = conv
          ? conv.id
          : (await createConversation(session.userId, requestedModel)).id;
      } else {
